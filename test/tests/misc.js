...
(function () {
	...
	tests = [
		...
		{
			name: 'If a select\'s value attribute is updated at the same time as the available options, the correct option will be selected',
			test: function () {
				var ractive;
				...
				equal( fixture.innerHTML, '<select id="select"></select>' );
	...
	runTest = function ( t, testNum ) {
		test( t.name, function () {
			console.group( testNum + 1 );
