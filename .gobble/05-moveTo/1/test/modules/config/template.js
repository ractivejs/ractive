...
function (
	...
	return function () {
		...
		var Ractive, Component, ractive,
			...
			templateOpt1fn = { template: function()  {return templateOpt1.template} },
		...
		test( 'Default create', function(t ) {
			testDefault( Ractive.defaults.template );
		});


		test( 'Empty extend inherits parent', function(t ) {
			mockExtend();
		...
		test( 'Extend twice with different templates', function(t ) {
		...
		test( 'Init with pure string template', function(t ) {
			config.init( Ractive, ractive, { template: 'foo' } );
			t.equal( ractive.template, 'foo' );
		});

		test( 'Init take precedence over default', function(t ) {
			config.extend( Ractive, Component.prototype, templateOpt1 );
		...
		test( 'Extend with template function', function(t ) {
		...
		test( 'Extend uses child parse options', function(t ) {
		...
		test( 'Init with template function', function(t ) {
		...
		test( 'Overwrite after extend before init', function(t ) {
		...
		test( 'Template function arguments and this', function(t ) {
		...
		test( 'Template with partial', function(t ) {
		...
		test( 'Template with partial extended', function(t ) {
		...
		test( 'Template with partial added and takes precedence over option partials', function(t ) {
