define([ 'ractive' ], function ( Ractive ) {
	...
	return function () {
		...
		test( 'on-click="someEvent" does not fire event when unrendered', function(t ) {
			var ractive, node;
		...
		test( 'custom event invoked and torndown', function(t ) {
		...
		test( 'Empty event names are safe, though do not fire', function(t ) {
		...
		test( 'Multiple space-separated events can be handled with a single callback (#731)', function(t ) {
			...
			ractive.on( ' foo bar  baz', function()  {return count += 1} );
		...
		test( 'ractive.off() is chainable (#677)', function(t ) {
		...
		function testEventBubbling( fire ) {
			...
			test( 'arguments bubble', function(t ) {
				var ractive, middle, component;
