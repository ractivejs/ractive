		...
		run ( opts ) {
			if (opts.directory) {
				...
				function step() {
					const f = files.pop();
					if (f) {
						util.readToString( fs.createReadStream( path.join( opts.directory, f ) ) ).then( string => {
							return component.build( string, opts, readFile ).
								then( string => {
									const file = path.join( opts.output, f.replace( ext, opts.outputExtension || '.js' ) );
...
if ( !opts.directory && typeof opts.output === 'string' ) opts.output = fs.createWriteStream( opts.output );
else if ( opts.directory && typeof opts.output !== 'string' ) {
	console.error( `Directory transforms require an output directory` );
	process.exit(2);
}
