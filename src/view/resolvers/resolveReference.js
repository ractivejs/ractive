...
export default function resolveReference(fragment, ref) {
	...
	if (ref[0] === '.' || ref[0] === '^') {
		...
		while (parts[0] === '^^') {
			...
			while (frag && !context) {
				...
				frag = frag.parent.component
					? frag.parent.component.parentFragment
					: frag.parent;
			}
		}
	...
	if (base[0] === '@') {
		...
		if (base === '@this' || base === '@') {
			...
			return repeater.context.getKeyModel(
				repeater[ref[1] === 'i' ? 'index' : 'key']
			);
		} else if (base === '@global') {
	...
	while (fragment) {
		...
		if (fragment.context && fragment.context.has(base)) {
			...
			if (createMapping) {
				...
				return context.root
					.createLink(base, fragment.context.joinKey(base), base, {
						implicit: true
					})
					.joinAll(keys);
			}
		...
		if (
			...
			fragment =
				fragment.componentParent || fragment.ractive.component.parentFragment;
			createMapping = true;
		} else {
