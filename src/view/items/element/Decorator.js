...
export default class Decorator {
	...
	render() {
		runloop.scheduleTask(() => {
			...
			if (!this.intermediary || !this.intermediary.teardown) {
				throw new Error(
					`The '${this
						.name}' decorator must return an object with a teardown method`
				);
			}
	...
	unrender(shouldDestroy) {
		if ((!shouldDestroy || this.element.rendered) && this.intermediary)
	...
	update() {
		...
		if (this.intermediary) {
			if (!this.intermediary.update) {
				...
				this.intermediary.update.apply(
					this.ractive,
					this.fn.apply(this.ractive, args)
				);
			}
		}
	}
