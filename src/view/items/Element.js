...
export default class Element extends ContainerItem {
	constructor(options) {
		...
		if (this.parent && this.parent.name === 'option') {
			throw new Error(
				`An <option> element cannot contain other elements (encountered <${this
					.name}>)`
			);
		}
	...
	render(target, occupants) {
		...
		if (this.parentFragment.cssIds) {
			node.setAttribute(
				'data-ractive-css',
				this.parentFragment.cssIds.map(x => `{${x}}`).join(' ')
			);
		}
	...
	toString() {
		...
		if (this.parentFragment.cssIds) {
			attrs += ` data-ractive-css="${this.parentFragment.cssIds
				.map(x => `{${x}}`)
				.join(' ')}"`;
		}
...
function delegateHandler(ev) {
	...
	while (bubble && node && node !== end) {
		listeners =
			node._ractive &&
			node._ractive.proxy &&
			node._ractive.proxy.listeners[name];
