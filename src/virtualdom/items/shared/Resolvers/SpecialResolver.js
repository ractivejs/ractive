...
SpecialResolver.prototype = {
	rebind: function () {
		...
		if ( this.cached ) {
			return this.callback( this.ractive.viewmodel.getKeypath( '@' + prop.prefix + getProp( this.cached, prop ) ) );
		}
		...
		if ( prop.prop.indexOf( 'index' ) !== -1 || prop.prop.indexOf( 'key' ) !== -1 ) {
			while ( fragment ) {
				if ( fragment.owner.currentSubtype === SECTION_EACH && ( value = getProp( fragment, prop ) ) !== undefined ) {
					...
					return this.callback( this.ractive.viewmodel.getKeypath( '@' + prop.prefix + value ) );
				}
			...
			while ( fragment ) {
				if ( ( value = getProp( fragment, prop ) ) !== undefined ) {
					return this.callback( this.ractive.viewmodel.getKeypath( '@' + prop.prefix + value.str ) );
				}
