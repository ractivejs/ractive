...
import { load } from '@ractivejs/loader-node'
import Ractive from '@ractivejs/core'

// Only needed to silence Ractive debugging
Ractive.DEBUG = false

// Node treats an absolute path as relative to the current drive it's running.
// We need to strip the drive and normalize before handing off to loader-node.
const underpWindows = p => p.replace(/[a-zA-Z]+:\\\\?/, '/').split(sep).join('/')

QUnit.module('loader-node')

QUnit.test('relative to caller', assert => {
  return load('test/samples/relative-caller/component.ractive.html').then(Component => {
    ...
    assert.strictEqual(instance.toHTML(), html)
  })
})

QUnit.test('relative to base', assert => {
  return load('component.ractive.html', { basePath: './test/samples/relative-base/', map: { answer: 42 } }).then(Component => {
    assert.strictEqual(typeof Component, 'function')
    assert.strictEqual(typeof Component.extend, 'function')
    assert.deepEqual(Component.defaults.data, { message: 'World', answer: 42 })
    assert.deepEqual(Component.defaults.template, { 'v': 4, 't': [{ 't': 7, 'e': 'div', 'f': ['Hello, ', { 't': 2, 'r': 'message' }, '!'] }, ' ', { 't': 7, 'e': 'div', 'f': ['Answer to the Ultimate Question of Life, the Universe, and Everything: ', { 't': 2, 'r': 'answer' }] }, ' ', { 't': 7, 'e': 'MyComponent' }] })
...
QUnit.test('relative to component', assert => {
  return load('./test/samples/relative-component/component.ractive.html', { map: { answer: 42 } }).then(Component => {
    assert.strictEqual(typeof Component, 'function')
    assert.strictEqual(typeof Component.extend, 'function')
    assert.deepEqual(Component.defaults.data, { message: 'World', answer: 42 })
    assert.deepEqual(Component.defaults.template, { 'v': 4, 't': [{ 't': 7, 'e': 'div', 'f': ['Hello, ', { 't': 2, 'r': 'message' }, '!'] }, ' ', { 't': 7, 'e': 'div', 'f': ['Answer to the Ultimate Question of Life, the Universe, and Everything: ', { 't': 2, 'r': 'answer' }] }, ' ', { 't': 7, 'e': 'MyComponent' }] })
...
QUnit.test('relative to component, no options', assert => {
  return load('./test/samples/relative-component-no-options/component.ractive.html').then(Component => {
    assert.strictEqual(typeof Component, 'function')
    assert.strictEqual(typeof Component.extend, 'function')
    assert.deepEqual(Component.defaults.data, { message: 'World', answer: 42 })
    assert.deepEqual(Component.defaults.template, { 'v': 4, 't': [{ 't': 7, 'e': 'div', 'f': ['Hello, ', { 't': 2, 'r': 'message' }, '!'] }, ' ', { 't': 7, 'e': 'div', 'f': ['Answer to the Ultimate Question of Life, the Universe, and Everything: ', { 't': 2, 'r': 'answer' }] }, ' ', { 't': 7, 'e': 'MyComponent' }] })
...
QUnit.test('relative to component, with base', assert => {
  return load('./component.ractive.html', { basePath: './test/samples/relative-component-with-base/', map: { answer: 42 } }).then(Component => {
    assert.strictEqual(typeof Component, 'function')
    assert.strictEqual(typeof Component.extend, 'function')
    assert.deepEqual(Component.defaults.data, { message: 'World', answer: 42 })
    assert.deepEqual(Component.defaults.template, { 'v': 4, 't': [{ 't': 7, 'e': 'div', 'f': ['Hello, ', { 't': 2, 'r': 'message' }, '!'] }, ' ', { 't': 7, 'e': 'div', 'f': ['Answer to the Ultimate Question of Life, the Universe, and Everything: ', { 't': 2, 'r': 'answer' }] }, ' ', { 't': 7, 'e': 'MyComponent' }] })
...
QUnit.test('relative to root', assert => {
  const componentAbsolutePath = underpWindows(resolve(__dirname, '../', 'test/samples/relative-root/component.ractive.html'))

  return load(componentAbsolutePath, { map: { answer: 42 } }).then(Component => {
    assert.strictEqual(typeof Component, 'function')
    assert.strictEqual(typeof Component.extend, 'function')
    assert.deepEqual(Component.defaults.data, { message: 'World', answer: 42 })
    assert.deepEqual(Component.defaults.template, { 'v': 4, 't': [{ 't': 7, 'e': 'div', 'f': ['Hello, ', { 't': 2, 'r': 'message' }, '!'] }, ' ', { 't': 7, 'e': 'div', 'f': ['Answer to the Ultimate Question of Life, the Universe, and Everything: ', { 't': 2, 'r': 'answer' }] }, ' ', { 't': 7, 'e': 'MyComponent' }] })
...
QUnit.test('relative to root, with base', assert => {
  const componentAbsolutePath = underpWindows(resolve(__dirname, '../', 'test/samples/relative-root-with-base/component.ractive.html'))

  return load(componentAbsolutePath, { basePath: '/foo/bar/baz/', map: { answer: 42 } }).then(Component => {
    assert.strictEqual(typeof Component, 'function')
    assert.strictEqual(typeof Component.extend, 'function')
    assert.deepEqual(Component.defaults.data, { message: 'World', answer: 42 })
    assert.deepEqual(Component.defaults.template, { 'v': 4, 't': [{ 't': 7, 'e': 'div', 'f': ['Hello, ', { 't': 2, 'r': 'message' }, '!'] }, ' ', { 't': 7, 'e': 'div', 'f': ['Answer to the Ultimate Question of Life, the Universe, and Everything: ', { 't': 2, 'r': 'answer' }] }, ' ', { 't': 7, 'e': 'MyComponent' }] })
...
QUnit.test('multi-link', assert => {
  return load('./test/samples/multi-link/component.ractive.html', { map: { answer: 42 } }).then(Component => {
    assert.strictEqual(typeof Component, 'function')
    assert.strictEqual(typeof Component.extend, 'function')
    assert.deepEqual(Component.defaults.data, { message: 'World', answer: 42 })
...
QUnit.test('duplicate require', assert => {
  return load('./test/samples/relative-component/component.ractive.html', { map: { shared: {} } }).then(Component => {
    assert.strictEqual(typeof Component, 'function')
    assert.strictEqual(typeof Component.extend, 'function')
...
QUnit.test('duplicate link href', assert => {
  return load('./test/samples/duplicate-link-href/component.ractive.html', { map: { shared: {} } }).then(Component => {
    assert.strictEqual(typeof Component, 'function')
    assert.strictEqual(typeof Component.extend, 'function')
...
QUnit.test('duplicate link name', assert => {
  return load('./test/samples/duplicate-link-name/component.ractive.html', { map: { shared: {} } }).then(Component => {
    assert.strictEqual(typeof Component, 'function')
    assert.strictEqual(typeof Component.extend, 'function')
...
QUnit.test('multi-load', assert => {
  const basePath = './test/samples/multi-load/'
  ...
  return promises.then(components => {
    ...
    const instance1 = components[0]()
    const cssId1 = components[0].defaults.cssId
    const html1 = html(1, cssId1)

    const instance2 = components[1]()
    const cssId2 = components[1].defaults.cssId
    const html2 = html(2, cssId2)

    assert.strictEqual(instance1.toHTML(), html1)
    assert.strictEqual(instance2.toHTML(), html2)
