...
const getMap = (source, file, script, lineOffset) => {
  const content = (script && script.code) || ''
  const map = (script && script.map) || getSourceMap({source, file, content})
  const offset = (script && script.code) ? lineOffset : 0
  return offsetMapStart(map, offset)
}

export const toConstructor = ({source, file, parts, resolver}) => {
  const component = { exports: {} }

  if (parts.script) {
    const code = parts.script.code
    const map = getInlineSourceMap(getMap(source, file, parts.script, 2))
...
export const toModule = ({source, file, parts}) => {
  const sections = [
  ...
  const map = getMap(source, file, parts.script, 2)
  return { code, map }
}
