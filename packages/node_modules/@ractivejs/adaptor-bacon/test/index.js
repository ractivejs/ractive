...
import {baconAdaptor} from '@ractivejs/adaptor-bacon'

QUnit.module('adaptor-bacon')

QUnit.test('Adaptor can detect an observable', t => {
  t.ok(baconAdaptor.filter(new Bacon.Observable()))
  t.ok(baconAdaptor.filter(new Bacon.EventStream(() => {})))
  t.ok(baconAdaptor.filter(new Bacon.Property('prop', () => {})))
  t.ok(baconAdaptor.filter(new Bacon.Bus()))
})

QUnit.test('Initialize using a Property', t => {
...
QUnit.test('Initialize using a Bus', t => {
...
QUnit.test('Initialize using a set Property', t => {
  const observable = Bacon.constant('qux')
  const instance = Ractive({
    adapt: [ baconAdaptor ],
    template: `{{ observable }}`
...
QUnit.test('Initialize using a set Bus', t => {
  const observable = new Bacon.Bus()
  const instance = Ractive({
    adapt: [ baconAdaptor ],
    template: `{{ observable }}`
...
QUnit.test('Bus updates Ractive', t => {
  const observable = new Bacon.Bus()
  const instance = Ractive({
    adapt: [ baconAdaptor ],
    data: { observable },
...
QUnit.test('Ractive should not update Property', t => {
  const observable = Bacon.constant('qux')
  const instance = Ractive({
    adapt: [ baconAdaptor ],
    data: { observable },
...
QUnit.test('Ractive updates Bus', t => {
  const observable = new Bacon.Bus()
  const instance = Ractive({
    adapt: [ baconAdaptor ],
    data: { observable },
...
QUnit.test('Resetting with an Observable', t => {
...
QUnit.test('Resetting with an Observer', t => {
