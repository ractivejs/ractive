...
export default function() {
  ...
  test("fragment should skip non-rendered items when searching for its next node (#2317)", t => {
    const r = new Ractive({
      el: fixture,
      template:
        "{{#if step === 3}}3{{/if}}{{#if step === 1}}1{{/if}}{{#if step === 2}}2{{/if}}text",
      data: { step: 1 }
    });
  ...
  test(`rendering a textOnlyMode template renders text only`, t => {
    const r = new Ractive({
      template: Ractive.parse(
        `no <elements or="attributes" /> or &amp; & entities <{{any}} foo="bar"> {{just}} text, [[refs]], and {{#if foo}}sections{{/if}}`,
        { textOnlyMode: true }
      ),
      data: {
        any: "any",
    ...
    t.equal(
      r.toText(),
      'no <elements or="attributes" /> or &amp; & entities <any foo="bar"> just text, refs, and sections'
    );
  });
  ...
  test(`comments are properly rendered if they exist in the template - #3105`, t => {
    const r = new Ractive({
      target: fixture,
      template:
        "{{#if first }}<!-- first -->{{/if}}<span>{{#if second}}a{{/if}} <!-- comment -->, why not</span><!-- last -->",
      stripComments: false,
      data: {
        first: true,
        second: true
    ...
    t.equal(
      fixture.innerHTML,
      "<!-- first --><span>a <!-- comment -->, why not</span><!-- last -->"
    );
