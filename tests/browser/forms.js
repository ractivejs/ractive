...
export default function() {
  ...
  test("textarea with html content and no bindings should render the html text as a normal textarea would (#2198)", t => {
    ...
    t.equal(
      ...
      '<div class="foo"><strong>bar</strong></div><p>WAT</p>'
    );
  });
  ...
  test("textareas without binding allow any template content (#2063)", t => {
    ...
    t.equal(
      fixture.querySelector("textarea").value,
      "<i>change1</i>change2 <strong>change3</strong>"
    );
  });
  ...
  test(`non-twoway selects should not reset to their attribute value on update unless actually dirty (#2965)`, t => {
    const r = new Ractive({
      el: fixture,
      template:
        '<label>{{foo}} <select twoway="false" value="{{bar}}"><option value="{{baz}}">baz</option><option value="2">2</option></select></label>',
      data: {
        foo: "a",
        bar: "1",
        baz: "1"
