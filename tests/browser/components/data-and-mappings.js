...
export default function() {
  ...
  test('Regression test for #317', t => {
    ...
    t.htmlEqual(
      fixture.innerHTML,
      '<ul><li>0: a</li><li>1: b</li><li>2: c</li><li>3: d</li></ul><p>a b c d</p>'
    );

    ractive.push('items', 'e');
    t.htmlEqual(
      fixture.innerHTML,
      '<ul><li>0: a</li><li>1: b</li><li>2: c</li><li>3: d</li><li>4: e</li></ul><p>a b c d e</p>'
    );

    ractive.splice('items', 2, 1);
    t.htmlEqual(
      fixture.innerHTML,
      '<ul><li>0: a</li><li>1: b</li><li>2: d</li><li>3: e</li></ul><p>a b d e</p>'
    );
    ...
    t.htmlEqual(
      fixture.innerHTML,
      '<ul><li>0: f</li><li>1: b</li><li>2: d</li><li>3: g</li></ul><p>f b d g</p>'
    );
  ...
  test('Children do not nuke parent data when inheriting from ancestors', t => {
    ...
    const Block = Ractive.extend({
      template:
        '<Widget thing="{{things.one}}"/><Widget thing="{{things.two}}"/><Widget thing="{{things.three}}"/>',
      components: { Widget }
    });
    ...
    t.deepEqual(ractive.get('things'), {
      one: { value: 1 },
      two: { value: 2 },
      three: { value: 3 }
    });
  ...
  test('Inline component attributes are passed through correctly', t => {
    const Widget = Ractive.extend({
      template:
        "<p>{{foo.bar}}</p><p>{{typeof answer}}: {{answer}}</p><p>I got {{string}} but type coercion ain't one</p><p>{{dynamic.yes}}</p>"
    });

    const ractive = new Ractive({
      el: fixture,
      template:
        '<Widget foo="{bar:10}" answer="42 " string="99 problems" dynamic="{yes:{{but}}}"/>',
      data: { but: 'no' },
      components: { Widget }
    });
    ...
    t.htmlEqual(
      fixture.innerHTML,
      "<p>10</p><p>number: 42</p><p>I got 99 problems but type coercion ain't one</p><p>no</p>"
    );

    ractive.set('but', 'maybe');
    t.htmlEqual(
      fixture.innerHTML,
      "<p>10</p><p>number: 42</p><p>I got 99 problems but type coercion ain't one</p><p>maybe</p>"
    );
  });
  ...
  test('Reference expressions default to two-way binding (#996)', t => {
    ...
    t.deepEqual(JSON.parse(output.innerHTML), [
      { name: 'Brian', age: 54 },
      { name: 'Angela', age: 30 }
    ]);
  });
    ...
    const ractive = new Ractive({
      ...
      components: {
        Tweedle: Ractive.extend({
          template:
            '{{#dee}}{{ word ? word : "lewis"}}{{/dee}} {{#dum}}{{ word? word : "carroll"}}{{/}}'
        })
      }
    });
  ...
  test(`shuffling a link to a link to a list doesn't blow the stack (#2699)`, t => {
    ...
    const cmp1 = Ractive.extend({
      template: '<cmp2 list="{{list}}" />',
      isolated: false
    });
    const cmp2 = Ractive.extend({
      template: '<cmp3 list="{{list}}" />',
      isolated: false
    });
  ...
  test(`shuffling a link to a link to a list updates correctly`, t => {
    ...
    const cmp2 = Ractive.extend({
      template: '{{#each list}}{{.}}{{/each}}<cmp3 list="{{list}}" />',
      isolated: false
    });
    const cmp3 = Ractive.extend({ isolated: false });
    const r = new Ractive({
  ...
  test(`component attributes can contain stringified jsonish values`, t => {
    ...
    t.equal(
      JSON.stringify(r.findComponent().get('json')),
      JSON.stringify({ foo: [1, 2, 3], bar: { baz: true }, bat: 'yep' })
    );
  });
  ...
  test(`mapped indices should update correctly on shuffle (#3042)`, t => {
    ...
    t.htmlEqual(
      fixture.innerHTML,
      '0 dave<button>rm</button>1 joe<button>rm</button>2 susan<button>rm</button>3 marie<button>rm</button>'
    );
    ...
    t.htmlEqual(
      fixture.innerHTML,
      '0 dave<button>rm</button>1 susan<button>rm</button>2 marie<button>rm</button>'
    );
