...
export default function() {
	...
	test(`parser transforms can drop elements`, t => {
		const parsed = Ractive.parse(`<foo /><div />`, {
			transforms: [n => (n.e === 'foo' ? { remove: true } : undefined)]
		});
	...
	test(`parser transforms can modify an element in place`, t => {
		const parsed = Ractive.parse(`<div />`, {
			transforms: [
				n =>
					n.e === 'div'
						? (n.m || (n.m = [])).push({ t: 13, f: 'bar', n: 'class' }) && n
						: undefined
			]
		});
		...
		t.deepEqual(parsed, {
			v: 4,
			t: [{ t: 7, e: 'div', m: [{ t: 13, f: 'bar', n: 'class' }] }]
		});
	});
	...
	test(`parser transforms can replace an element`, t => {
		const parsed = Ractive.parse(`<bar />`, {
			transforms: [
				function(n) {
					return n.e === 'bar'
						? { replace: this.parse(`<div id="replaced">yep</div>`).t[0] }
						: undefined;
				}
			]
		});
		...
		t.deepEqual(parsed, {
			v: 4,
			t: [
				{ t: 7, e: 'div', m: [{ t: 13, f: 'replaced', n: 'id' }], f: ['yep'] }
			]
		});
	});
			...
			transforms: [
				function(n) {
					return n.e === 'bar'
						? {
								replace: this.parse(
									`<div id="replaced">yep</div><span>sure</span>`
								).t
							}
						: undefined;
				}
			...
			transforms: [
				function(n) {
					return n.e === 'bar'
						? {
								replace: this.parse(
									`<div id="replaced">yep</div><baz>sure</baz>`
								).t
							}
						: undefined;
				},
	...
	test(`parser transforms contribute to the parsed expression map`, t => {
		const parsed = Ractive.parse(`<bar />`, {
			transforms: [
				function(n) {
					return n.e === 'bar'
						? {
								replace: this.parse(`<div id="{{'' + replaced}}">yep</div>`)
									.t[0]
							}
						: undefined;
				}
		...
		t.deepEqual(parsed.t, [
			{
				t: 7,
				e: 'div',
				m: [
					{ t: 13, f: [{ t: 2, x: { s: `""+_0`, r: ['replaced'] } }], n: 'id' }
				],
				f: ['yep']
			}
		]);
	...
	test(`parser transforms may be global`, t => {
		Ractive.defaults.parserTransforms.push(function(n) {
			return n.e === 'bar'
				? { replace: this.parse(`<div id="replaced">yep</div>`).t[0] }
				: undefined;
		});
	...
	test(`parser transforms include local and global options`, t => {
		Ractive.defaults.parserTransforms.push(function(n) {
			return n.e === 'bar'
				? { replace: this.parse(`<baz id="replaced">yep</baz>`).t[0] }
				: undefined;
		});
	...
	test(`parser transforms also apply to inline partials`, t => {
		const parsed = Ractive.parse(
			`<div>{{#partial foo}}<bar />{{/partial}}</div>`,
			{
				transforms: [
					function(n) {
						return n.e === 'bar'
