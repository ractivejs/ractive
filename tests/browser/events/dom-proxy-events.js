...
export default function() {
  ...
  test("event keypath is set to the mapped keypath in a component", t => {
    ...
    const cmp = Ractive.extend({
      template:
        '{{#with baz}}<span id="test" on-click="someEvent">click me</span>{{/with}}<cmp2 oof="{{baz}}" />',
      isolated: false
    });
  ...
  test("events can correctly retrieve index refs", t => {
    ...
    const ractive = new Ractive({
      el: fixture,
      template:
        '<ul>{{#array:i}}<li id="item_{{i}}" on-click="someEvent">{{i}}: {{.}}</li>{{/array}}</ul>',
      data: {
        array: ["a", "b", "c", "d", "e"]
      }
    });
  ...
  test("events can correctly retrieve nested index refs", t => {
    ...
    const ractive = new Ractive({
      el: fixture,
      template:
        '{{#foo:x}}{{#bar:y}}{{#baz:z}}<span id="test_{{x}}{{y}}{{z}}" on-click="someEvent">{{x}}{{y}}{{z}}</span>{{/baz}}{{/bar}}{{/foo}}',
      data: {
        foo: [
          {
            bar: [{ baz: [1, 2, 3] }]
  ...
  test("Superfluous whitespace is ignored", t => {
    const ractive = new Ractive({
      el: fixture,
      template:
        '<div class="one" on-click=" foo "></div><div class="two" {{#bar}}on-click=" bar "{{/}}></div>'
    });
  ...
  test('component "on-" supply own event proxy arguments (but original args are tacked on)', t => {
    ...
    const ractive = new Ractive({
      el: fixture,
      template:
        '<Component on-foo="@this.fire("foo-reproxy", event, 1)" on-bar="@this.fire("bar-reproxy", "qux")" on-bizz="bizz-reproxy"/>',
      data: {
        qux: "qux"
      },
