...
export default function() {
  ...
  test('event directives should shuffle correctly', t => {
    ...
    const r = new Ractive({
      el: fixture,
      template:
        '{{#each items}}<div id="div{{@index}}" on-click="@this.fire("foo", event, .bar)" />{{/each}}',
      data: {
        items: [{ bar: 'baz' }]
      }
    });
  ...
  test('method event directives should shuffle correctly', t => {
    ...
    const r = new Ractive({
      el: fixture,
      template:
        '{{#each items}}<div id="div{{@index}}" on-click="@this.foo(@keypath, .bar)" />{{/each}}',
      data: {
        items: [{ bar: 'baz' }]
      },
      foo(path, bar) {
  ...
  test(`shuffled elements with attributes depending on template context (@index, etc) should have appropriately updated attributes (#2422)`, t => {
    ...
    for (let i = 0; i < spans.length; i++)
      t.equal(spans[i].getAttribute('data-wat'), spans[i].innerHTML);
  ...
  test(`shuffling lists with nested lists with same-named keypaths (#2491)`, t => {
    ...
    t.equal(
      fixture.innerHTML,
      '<span>list.0.list.0</span><span>list.1.list.0</span><span>list.1.list.1</span><span>list.1.list.2</span>'
    );
    const tmp = r.splice('list.1.list', 0, 1).result[0];
    t.equal(
      fixture.innerHTML,
      '<span>list.0.list.0</span><span>list.1.list.0</span><span>list.1.list.1</span>'
    );
    r.splice('list', 0, 0, tmp);
    t.equal(
      fixture.innerHTML,
      '<span>list.0.list.0</span><span>list.1.list.0</span><span>list.2.list.0</span><span>list.2.list.1</span>'
    );
  ...
  test(`shuffle set should also check for element updates (#3010)`, t => {
    ...
    t.htmlEqual(
      fixture.innerHTML,
      '<div>apple</div><div>filler1</div><div>filler2</div><div>orange</div><div>banana</div>'
    );
    ...
    t.htmlEqual(
      fixture.innerHTML,
      '<div>gwen stefani</div><div>george lucas</div><div>granny smith</div>'
    );
  });
