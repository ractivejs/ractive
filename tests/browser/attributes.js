...
export default function() {
	...
	test(`style directives and style attributes both contribute to toHTML (#2537)`, t => {
		...
		t.equal(
			r.toHTML(),
			`<span style="text-decoration: underline; color: green; font-size: 12pt;"></span><span style="text-decoration: underline; font-size: 12pt;"></span>`
		);
	});
	...
	test(`class directives work with the weird classes on svg elements (#2955)`, t => {
		new Ractive({
			el: fixture,
			template:
				'<svg><rect x="0" y="0" width="100" height="100" class="red" class-bordered /></svg>'
		});

		t.htmlEqual(
			fixture.innerHTML,
			'<svg><rect x="0" y="0" width="100" height="100" class="red bordered" /></svg>'
		);
	});
}
