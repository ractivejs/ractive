...
export default function() {
  ...
  test('Two-way bindings work with arbitrary expressions that resolve to keypaths', t => {
    const ractive = new Ractive({
      el: fixture,
      template:
        '<label><input value={{foo["bar"][ a+1 ].baz[ b ][ 1 ] }}> {{ foo.bar[1].baz.qux[1] }}</label>',
      data: {
        foo: {
          bar: [null, { baz: { qux: [null, 'yes'] } }]
        },
        a: 0,
        b: 'qux'
      }
  ...
  test('Two-way data binding is not attempted on elements with no mustache binding', t => {
    ...
    new Ractive({
      el: fixture,
      template:
        '<input type="radio"><input type="checkbox"><input type="file"><select></select><textarea></textarea><div contenteditable="true"></div>'
    });
  });
  ...
  test('Contenteditable elements can be bound via the value attribute', t => {
    const ractive = new Ractive({
      el: fixture,
      template:
        '<div contenteditable="true" value="{{content}}"><strong>some content</strong></div>'
    });

    t.equal(ractive.get('content'), '<strong>some content</strong>');
    t.htmlEqual(
      fixture.innerHTML,
      '<div contenteditable="true"><strong>some content</strong></div>'
    );

    ractive.set('content', '<p>some different content</p>');
    t.htmlEqual(
      fixture.innerHTML,
      '<div contenteditable="true"><p>some different content</p></div>'
    );
  });
  ...
  try {
    ...
    test('Contenteditable elements can be bound with a bindable contenteditable attribute.', t => {
      const ractive = new Ractive({
        el: fixture,
        template:
          '<div contenteditable="{{editable}}" value="{{content}}"><strong>some content</strong></div>',
        data: { editable: false }
      });
  ...
  test('The order of attributes does not affect contenteditable (#1134)', t => {
    ...
    t.htmlEqual(
      fixture.innerHTML,
      '<div contenteditable="true">one</div><div contenteditable="true">two</div>'
    );
  });
  ...
  ['number', 'range'].forEach(type => {
    test('input type=' + type + ' values are coerced', t => {
      ...
      t.htmlEqual(
        fixture.innerHTML,
        '<input type="' + type + '"><input type="' + type + '">40+2=42'
      );
    });
  });
  ...
  test('The model updates to reflect which checkbox inputs are checked at render time', t => {
    let ractive = new Ractive({
      el: fixture,
      template:
        '<input id="red" type="checkbox" name="{{colors}}" value="red"><input id="green" type="checkbox" name="{{colors}}" value="blue" checked><input id="blue" type="checkbox" name="{{colors}}" value="green" checked>'
    });
    ...
    ractive = new Ractive({
      el: fixture,
      template:
        '<input id="red" type="checkbox" name="{{colors}}" value="red"><input id="green" type="checkbox" name="{{colors}}" value="blue"><input id="blue" type="checkbox" name="{{colors}}" value="green">'
    });
  ...
  test('Checkbox Name bindings use property attributes to determined if checked or not', t => {
    let ractive = new Ractive({
      ...
      template:
        '{{#each colors}}<input id="{{id}}" type="checkbox" name="{{selected}}" value-comparator="id" value="{{.}}">{{/each}}'
    });
    ...
    ractive = new Ractive({
      ...
      template:
        '{{#each colors}}<input id="{{id}}" type="checkbox" name="{{selected}}" value-comparator="{{@this.comparatorFn}}" value="{{.}}">{{/each}}'
    });
    t.deepEqual(ractive.get('selected'), [{ id: 'green' }]);
    t.ok(!ractive.find('#red').checked);
    t.ok(!ractive.find('#blue').checked);
  ...
  test('Radio Name bindings use value-comparator to determined if checked or not', t => {
    let ractive = new Ractive({
      ...
      template:
        '{{#each colors}}<input id="{{id}}" type="radio" name="{{selected}}" value-comparator="id" value="{{.}}">{{/each}}'
    });
    t.deepEqual(ractive.get('selected'), { id: 'green' });
    t.ok(!ractive.find('#red').checked);
    t.ok(!ractive.find('#blue').checked);
    ...
    ractive = new Ractive({
      ...
      template:
        '{{#each colors}}<input id="{{id}}" type="radio" name="{{selected}}" value-comparator="{{@this.comparatorFn}}" value="{{.}}">{{/each}}'
    });
    t.deepEqual(ractive.get('selected'), { id: 'green' });
    t.ok(!ractive.find('#red').checked);
    t.ok(!ractive.find('#blue').checked);
  ...
  test('The model updates to reflect which radio input is checked at render time', t => {
    let ractive = new Ractive({
      el: fixture,
      template:
        '<input type="radio" name="{{color}}" value="red"><input type="radio" name="{{color}}" value="blue" checked><input type="radio" name="{{color}}" value="green">'
    });
    ...
    ractive = new Ractive({
      el: fixture,
      template:
        '<input type="radio" name="{{color}}" value="red"><input type="radio" name="{{color}}" value="blue"><input type="radio" name="{{color}}" value="green">'
    });
  ...
  test('The model overrides which radio input is checked at render time', t => {
    const ractive = new Ractive({
      el: fixture,
      template:
        '<input id="red" type="radio" name="{{color}}" value="red"><input id="blue" type="radio" name="{{color}}" value="blue" checked><input id="green" type="radio" name="{{color}}" value="green">',
      data: { color: 'green' }
    });
  ...
  test('updateModel correctly updates the value of a select', t => {
    const ractive = new Ractive({
      el: fixture,
      template:
        '<select value="{{selected}}"><option selected value="red">red</option><option value="blue">blue</option><option value="green">green</option></select>'
    });
  ...
  test('updateModel correctly updates the value of an indirect (name-value) checkbox', t => {
    const ractive = new Ractive({
      el: fixture,
      template:
        '<input type="checkbox" name="{{colour}}" value="red"><input type="checkbox" name="{{colour}}" value="blue" checked><input type="checkbox" name="{{colour}}" value="green">'
    });
  ...
  test('updateModel correctly updates the value of an indirect (name-value) radio', t => {
    const ractive = new Ractive({
      el: fixture,
      template:
        '<input type="radio" name="{{colour}}" value="red"><input type="radio" name="{{colour}}" value="blue" checked><input type="radio" name="{{colour}}" value="green">'
    });
  ...
  test('Changes made in oninit are reflected on render (#1390)', t => {
    ...
    new Ractive({
      el: fixture,
      template:
        '{{#each items}}<input type="checkbox" name="{{selected}}" value="{{this}}">{{/each}}',
      data: {
        items: ['a', 'b', 'c'],
  ...
  test('Two-way binding can be set up against expressions that resolve to regular keypaths', t => {
    const ractive = new Ractive({
      el: fixture,
      template:
        '{{#items:i}}<label><input value="{{ proxies[i].name }}"> name: {{ proxies[i].name }}</label>{{/items}}',
      data: {
        items: [{}],
        proxies: []
      }
  ...
  test('select with no matching value option selects none (#2494)', t => {
    const r = new Ractive({
      el: fixture,
      template:
        '<select value="{{foo}}">{{#if opt1}}<option>1</option>{{/if}}<option>2</option></select>',
      data: { opt1: true }
    });
  ...
  test('binding to an reference proxy does not cause out-of-syncitude with the actual model', t => {
    const r = new Ractive({
      el: fixture,
      template:
        '<span>{{foo.bar.baz}}</span>{{#with foo[what]}}<input value="{{.baz}}" />{{/with}}',
      data: {
        foo: {
          bar: { baz: 'yep' },
          bat: { baz: 'also yep' }
  ...
  test('bindings stay up to date when updated from the model (#2643)', t => {
    ...
    r.observe('foo', v => t.ok(v, 'new value true observed from click'), {
      init: false
    });
    fire(input, 'click');
    t.ok(input.checked, 'input checked by click');
  ...
  test(`options that exist across a component boundary register with their parent select correctly`, t => {
    const cmp = Ractive.extend({
      template: '<select value="{{foo}}">{{yield}}</select>'
    });
    const r = new Ractive({
      el: fixture,
  ...
  test(`radio name bindings survive a shuffle (#2939)`, t => {
    ...
    const r = new Ractive({
      ...
      data: {
        xs: [{ selected: 1 }, { selected: 2 }, { selected: 1 }, { selected: 3 }]
      }
    });
