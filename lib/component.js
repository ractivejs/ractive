		...
		if ( item.t === 7 ) {
			if ( item.e === 'script' ) {
				...
				if ( id && ( type === 'text/ractive' || type === 'text/html' ) ) {
					...
					if ( rel === 'css' ) {
						if ( cssFn ) return Promise.reject( `Only one css script may be specified per component.` );
...
function compileCss( styles, fn, opts ) {
	if ( !styles.length || !styles.join( '' ) ) return Promise.resolve( '""' );

	if ( fn ) {
		const promises = [];
		const left = styles.slice( 0, styles.indexOf( 'style!fn' ) );
		const right = styles.slice( styles.indexOf( 'style!fn' ) + 1 );

		if ( left.length ) promises.push( reducePromiseFunctions( opts.styleProcessors, left.join( '' ) ).then( css => css.replace( /\s+/g, ' ' ) ) );
		if ( right.length ) promises.push( reducePromiseFunctions( opts.styleProcessors, right.join( '' ) ).then( css => css.replace( /\s+/g, ' ' ) ) );

		let indent = /^\s+/.exec( fn.body );
		...
		return Promise.all(promises).then( result => {
			const styles = result.map( css => JSON.stringify( css ) );
			if ( styles.length ) {
				func = `(${func}).call(this, ${fn.name}) || ''`;
				if ( left.length ) func = JSON.stringify( result[0] ) + ',\n' + func;
				if ( right.length ) func = func + ',\n' + JSON.stringify( result[ left.length ? 1 : 0 ] );

				return `function(${fn.name}) { return [\n${func}\n].join(''); }`;
			} else {
				return  func;
			}
		});
	} else {
		return reducePromiseFunctions( opts.styleProcessors, styles.join( '' ) ).then( css => css.replace( /\s+/g, ' ' ) );
	}
}
