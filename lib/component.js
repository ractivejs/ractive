...
function build ( string, opts, readFile ) {
	...
	rootOpts.interpolate = Object.assign({}, opts.interpolate, { script: false, style: false, template: false });
	...
	return Promise.all( promises ).then( () => {
		...
		if (!script && opts.autoExport) script = `${tpl ? `export const template = $TEMPLATE;\n` : ''}${style.length ? `export const css = $CSS;\n` : ''}`;
